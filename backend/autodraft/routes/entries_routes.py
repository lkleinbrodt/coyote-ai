...
def _add_response(prompt_id, text):
    prompt = Prompt.query.get(prompt_id)
    if not prompt:
...
def update_response():
    ...
    response = _update_response(response.prompt_id, response_id, text, selected)
    return (
...
def _update_response(prompt_id, response_id, text, selected=True):
    prompt = Prompt.query.get(prompt_id)
    response = Response.query.get(response_id)
    response.text = text
...
def generate_all():
    ...
    for prompt in prompts:
        ...
        if existing_response:
            new_response = _update_response(prompt.id, existing_response.id, response)
        else:
...
def write():
    ...
    if existing_response:
        new_response = _update_response(prompt_id, existing_response.id, response)
    else:
        new_response = _add_response(prompt_id, response)
