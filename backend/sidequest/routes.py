    ...
    try:
        ...
        for field in required_fields:
            if field not in preferences:
                ...
                return jsonify({
                    "success": False,
                    "error": {"message": f"Missing required preference field: {field}"}
                }), 400
        
        # Validate categories
        if not isinstance(preferences["categories"], list) or not preferences["categories"]:
        ...
        try:
            ...
            return jsonify({
                "success": False,
                "error": {"message": "Invalid difficulty level"}
            }), 400
        ...
        logger.info(f"Generating quests for user {user_id} with preferences: {preferences}")
        ...
        return jsonify({
            "success": False,
            "error": {"message": "Failed to generate quests"}
        }), 500
...
def get_user_quests():
    ...
    try:
        ...
        logger.info(f"Fetching quests for user {user_id}, date: {date}, include_expired: {include_expired}")
...
def select_quest(quest_id: int):
    ...
    try:
        ...
        if not success:
            logger.warning(f"Quest {quest_id} not found or not accessible to user {user_id}")
            return jsonify({
                "success": False,
        ...
        return jsonify({
            "success": False,
            "error": {"message": "Failed to select quest"}
        }), 500
...
def complete_quest(quest_id: int):
    ...
    try:
        ...
        logger.info(f"User {user_id} completing quest {quest_id} with feedback: {feedback}")
...
def skip_quest(quest_id: int):
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": {"message": "Failed to skip quest"}
        }), 500
...
def get_quest_history():
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                "success": False,
                "error": {"message": "Days must be a positive integer between 1 and 365"}
            }), 400
...
def health_check():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": {"message": "Service unhealthy"}
        }), 500
