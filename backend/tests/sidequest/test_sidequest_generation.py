...
class TestQuestGenerationService:
    ...
    def test_fallback_quests_initialization(self, app):
        ...
        with app.app_context():
            service = QuestGenerationService(app.db.session, "test_key")
    ...
    @patch('openai.OpenAI')
    def test_generate_with_llm_success(self, mock_openai, app):
    ...
    @patch('openai.OpenAI')
    def test_generate_with_llm_failure(self, mock_openai, app):
    ...
    @patch('openai.OpenAI')
    def test_generate_daily_quests_llm_success(self, mock_openai, app):
        ...
        with app.app_context():
            ...
            user_profile = SideQuestUser(user_id=1)
            app.db.session.add(user_profile)
            app.db.session.commit()
            
            quests = service.generate_daily_quests(1, preferences)
    ...
    @patch('openai.OpenAI')
    def test_generate_daily_quests_fallback(self, mock_openai, app):
